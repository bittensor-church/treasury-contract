...
def main():
    parser = argparse.ArgumentParser(description="Propose & Vote Batch: Transfer Stake")
    ...
    print(f"Submitting Batch Proposal & Vote with {len(targets)} actions...")
    ...
    fn = governor.functions.proposeAndVote(targets, values, calldatas, args.description)
    ...
    with warnings.catch_warnings():
        ...
        if logs:
            print(f"✅ Proposal Created! ID: {logs[0]['args']['proposalId']}")

        vote_logs = governor.events.VoteCast().process_receipt(receipt)
        if vote_logs:
            print(f"✅ Vote Cast Immediately! Support: {vote_logs[0]['args']['support']} (1=For)")
