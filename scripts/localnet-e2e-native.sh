...
DEPLOYER_ADDR="${DEPLOYER_ADDR:-0x7bD3E0F025FC388e08dd2A63595dbcaB486F335b}"
DEPLOYER_PK="${DEPLOYER_PK:-0x58a595a0863f6894cf22d465014abf7c7ca5b46fc8dd7e7e932d158002c33039}"
...
warn "Known audit K-7: associate_evm.py may fail due to outdated pallet ABI."

set +e
COLDKEY_SS58_ALICE=$(python3 -c "import json; print(json.load(open('$ALICE_DIR/coldkeypub.txt'))['ss58Address'])")
ASSOCIATE_OUTPUT=$(python3 "$PROJECT_ROOT/tools/associate_evm.py" \
    --rpc-url "$RPC_URL" \
    --private-key "$DEPLOYER_PK" \
    --coldkey "$COLDKEY_SS58_ALICE" \
    --hotkey "$HOTKEY_SS58" \
    --coldkey-seed "$ALICE_COLDKEY_SEED" 2>&1)
ASSOC_STATUS=$?
set -e
...
if [[ "$ASSOC_STATUS" -eq 0 ]]; then
    ...
    warn "associate_evm failed (see K-7). Continuing — voter may have 0 voting power."
...
log "Phase 6: Fund vault with $VAULT_FUND_AMOUNT TAO"
